...
const Homepage = () => {
  ...
  const getCardStyle = (hovered, isService = true) => ({
    ...
    textAlign: 'center',
    boxShadow: hovered ? '0 15px 25px rgba(0,0,0,0.4)' : '0 4px 6px rgba(0,0,0,0.1)',
  ...
  return (
    <div style={{ fontFamily: 'sans-serif', margin: 0, padding: 0 }}>
      ...
      >
        <div style={{ position: 'absolute', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 10 }}></div>
        ...
        <div style={{ position: 'relative', zIndex: 20, textAlign: 'center' }}>
          ...
          <button
            ...
            onMouseEnter={e => { e.target.style.backgroundColor = '#5DADEC'; e.target.style.boxShadow = '0 15px 25px rgba(0,0,0,0.4)'; }}
            onMouseLeave={e => { e.target.style.backgroundColor = '#7353BA'; e.target.style.boxShadow = '0 10px 15px rgba(0,0,0,0.2)'; }}
          >
            Get Started
          </button>
        </div>
      ...
      {/* Services Section */}
      <section id="services" style={{ backgroundColor: '#EBF5FF', padding: '6rem 1rem', margin: 0 }}>
        <div style={{ maxWidth: '1280px', margin: '0 auto', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '2rem', padding: '0 2rem' }}>
            {services.map((service, idx) => (
              ...
              >
                <div style={{ fontSize: '2rem', marginBottom: '0.5rem', color: 'white' }}>{service.icon}</div>
                <h3 style={{ color: '#7353BA', fontWeight: '600', marginBottom: '0.5rem' }}>{service.title}</h3>
                <p style={{ color: '#E5E7EB', fontSize: '0.875rem' }}>{service.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* About Section */}
      <section id="about" style={{ backgroundColor: 'white', padding: '6rem 1rem', margin: 0 }}>
        <div style={{ maxWidth: '960px', margin: '0 auto', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '0 1rem' }}>
          <div
            data-aos="fade-right"
            style={{
          ...
          >
            <h3 style={{ fontSize: aboutTitleSize, fontWeight: 'bold', marginBottom: '1rem' }}>About Homeiq</h3>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '1.5rem' }}>
            {testimonials.map((t, idx) => (
              ...
              >
                <p style={{ color: '#27293D', fontSize: '0.875rem', marginBottom: '0.5rem' }}>{t.text}</p>
                <div style={{ display: 'flex', alignItems: 'center', marginBottom: '0.25rem' }}>
                  <span style={{ color: '#FBBF24' }}>{t.rating}</span>
                </div>
                <h4 style={{ fontWeight: '600', color: '#5AC18E' }}>{t.author}</h4>
              </div>
            ))}
          </div>
        </div>
